  ...
  <div :class="['min-h-screen w-full max-w-none transition-all duration-500 ease-in-out',
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        <div 
          ...
          class="bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg overflow-hidden transform hover:scale-105 transition-all duration-300 ease-in-out cursor-pointer shadow-lg"
        >
          <div class="relative">
          ...
          <div class="p-4">
            <h3 class="text-lg font-semibold text-white mb-1">{{ document.title }}</h3>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 backdrop-filter backdrop-blur-sm flex items-center justify-center z-50" @click="closePreview">
      <div class="bg-white dark:bg-gray-800 rounded-lg p-4 md:p-8 max-w-2xl w-full m-4" @click.stop>
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Author: {{ currentDocument.author }}</p>
        <button @click="closePreview" class="mt-4 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-500 transition duration-300">
          Close
        </button>
      </div>
    </div>
