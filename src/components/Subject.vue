...
<template>  
    <div :class="['subject-view-page', { 'dark-mode': isDarkMode }]">  
     ...
     <div v-for="subject in subjects" :key="subject" class="subject-section">  
      <h2 class="section-title">{{ subject }}</h2>  
      <div class="documents-container">  
        <div class="documents-grid">  
         <div  
          v-for="(document, index) in filteredDocuments[subject]"  
          :key="index"  
          class="document-card"  
      ...
      <button class="view-all-btn"  @click="viewAllDocuments(subject)">View All {{ subject }} Documents</button>  
     </div>  
  ...
  const subjects = ref([]); // Add a ref for subjects  
  ...
  const filteredDocuments = computed(() => {  
    ...
    for (const subject in subjects.value) {  
     filtered[subject] = documents.value[subject].filter(document =>  
  ...
  const limitedDocuments = computed(() => {  
    const limited = {};  
    for (const subject in filteredDocuments.value) {  
     limited[subject] = filteredDocuments.value[subject].slice(0, 4);  
    }  
    return limited;  
  });  
    
  function viewAllDocuments(subject) {  
    router.push({ name: 'DocumentsBySubject', params: { subject } });  
  }  
  ...
  async function fetchDocuments() {  
    try {  
     ...
     const response = await axios.get(`${import.meta.env.VITE_API_URL}api/v1/documents`, { headers });  
     documents.value = mapDocuments(response.data.documents);
     subjects.value = Object.keys(response.data.documents); // Get subjects from response  
